...
int main(int argc, char *argv[])
{
    ...
    constexpr int arraysize = 10000000 * stride;
    ...
    // Create MPI vector datatype
    MPI_Type_vector(msgsize, 1, stride, MPI_INT, &dtype);
    // MPI_Type_contiguous(msgsize, MPI_INT, &dtype);
    ...
    for (int r=0; r < 10; r++) 
      {
        t0 = MPI_Wtime();
        MPI_Sendrecv(message.data(), 1, dtype, dest, 0, 
                     receiveBuffer.data(), 1, dtype, src, 0, 
    ...
    for (int r=0; r < 10; r++) 
      {
        t0 = MPI_Wtime();
        MPI_Irecv(receiveBuffer.data(), 1, dtype, src, tag, MPI_COMM_WORLD, &req[0]);
        //tmpi1 += MPI_Wtime() - t0;
        // res = compute();
        usleep(sleep_time);
        MPI_Isend(message.data(), 1, dtype, dest, tag, MPI_COMM_WORLD, &req[1]);
        //t0 = MPI_Wtime();
    ...
    for (int r=0; r < 10; r++) 
      {
        t0 = MPI_Wtime();
        MPI_Put(message.data(), 1, dtype, dest, 0, 1, dtype, rma_window);
        tmpi1 += MPI_Wtime() - t0;
        t0 = MPI_Wtime();
        res = compute();
        tcomp += MPI_Wtime() - t0;
    ...
    // tag = MPI_ANY_TAG;
    tag = 3;
    MPI_Recv_init(receiveBuffer.data(), 1, dtype, src, tag, MPI_COMM_WORLD, &req[0]);
    MPI_Send_init(message.data(), 1, dtype, dest, tag, MPI_COMM_WORLD, &req[1]);
    MPI_Barrier(MPI_COMM_WORLD);
    // Overlap
    for (int r=0; r < 10; r++) 
      {
